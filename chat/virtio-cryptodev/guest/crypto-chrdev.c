...
static int crypto_chrdev_open(struct inode *inode, struct file *filp)
{
	...
	int *host_fd; /* μάλλον μπορεί να βγει */
...
/*
 * int crypto_chrdev_release(struct inode *inode, struct file *filp);
 *
 * Καλείται όταν όλα τα copies ενός αρχείου έχουν κλείσει,
 * οπότε καταστρέφεται η αντίστοιχη δομή file.
 */
...
/*
 * long crypto_chrdev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
 *
 * Η ioctl προσφέρει έναν τρόπο για να κληθούν device-specific commands.
 *
 * @cmd: Το command που θέλουμε να περάσουμε στο ioctl
 * @arg: Τα ορίσματα που θέλουμε να χρησιμοποιήσουμε
 */
	...
	switch (cmd) {
		case CIOCGSESSION:
			...
			*ioctl_cmd = VIRTIO_CRYPTODEV_SYSCALL_IOCTL_CIOCGSESSION;
			...
			saved_key = session_op->key; /* map the key to user space later */
...
/*
 * ssize_t crypto_chrdev_read(struct file *filp, char __user *usrbuf, size_t cnt, loff_t *f_pos);
 *
 * Λαμβάνει δεδομένα από τη συσκευή.
 */
...
int crypto_chrdev_init(void)
{
	 ...
	 * dev_t
	 *
	 * Κρατάει τα major και minor device numbers.
	 */
	dev_t dev_no;
	 ...
	 *
	 * Κάνει initialize το cdev structure, και κάνει τον
	 * δείκτη fops να δείχνει στο file_operations που μας
	 * ενδιαφέρει.
	 */
	 ...
	 * MKDEV
	 *
	 * Macro που παίρνει ως είσοδο τα major και minor
	 ...
	 *
	 * Κάνει allocate το range των device numbers που χρειαζόμαστε.
	 */
	 ...
	 *
	 * Προσθέτει το char device στο σύστημα.
	 */
...
out_with_chrdev_region:
	 ...
	 *
	 * Απελευθερώνει το range των device numbers όταν δεν είναι
	 * πλέον υπό χρήση.
	 */
 ...
 *
 * Καλείται όταν το module γίνεται exit, οπότε "καταστρέφεται"
 * ο driver.
 */
void crypto_chrdev_destroy(void)
{
